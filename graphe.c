...
int paires(grapheReseau g, int i, int j, int seuil) {
    ...
    if (g == NULL) {
        ...
        return 0;  // ✅ return 0 et non return vide
    }
    ...
    while (courant != NULL) {   // ✅ tester courant != NULL et non courant->info
        ...
        courant = courant->suivant;  // ✅ avancer le curseur
    }
    ...
    if (g->articles[i]->score_fiabilite < seuil &&
        ...
        return 1;   // ✅ paire suspecte confirmée
    }
...
void chainecitationsSuspectes(grapheReseau g, int seuil) {
    ...
    for (int i = 0; i < g->V; i++) {             // ✅ g->V majuscule
        for (int j = 0; j < g->V; j++) {         // ✅ corps non vide
            if (i == j) continue;                 // éviter i==j
            if (paires(g, i, j, seuil)) {         // ✅ g et seuil passés en argument
